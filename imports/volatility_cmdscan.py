...
def volatility_cmdscan():
    ...
    while True:
        ...
        check_call(["clear"])
        print (logo)
        colorprint("info","'volatility' will be used to look for CMD command history buffers.")
        ...
        if err.find("The requested file doesn't exist") != -1:
            ...
            if out.find("No") != -1:
                ...
                std = Popen("volatility -f " + file_path + " --profile" + out + " cmdscan", shell=True, stdout=PIPE,stderr=PIPE)
                (out, err) = std.communicate()
